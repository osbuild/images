...
"test-distro" with version "1" has its image definitions
under the subdirectory "test-distro-1".
...
The name can container go templates and the following variables are
...
distribution. In the example above `match` will match `rhel-10.2`
and from that match the distribution name will be dynamically
constructed.
