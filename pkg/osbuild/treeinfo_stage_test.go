...
func TestNewTreeinfoStage(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			options, _ := actualMap["options"].(map[string]any)
			treeinfo, _ := options["treeinfo"].(map[string]any)
			tree, _ := treeinfo["tree"].(map[string]any)
			delete(tree, "build_timestamp")
			general, _ := treeinfo["general"].(map[string]any)
			delete(general, "timestamp")
