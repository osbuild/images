					...
					Config: osbuild.SystemdUnit{
						...
						Service: &osbuild.ServiceSection{
							...
							ExecStart: []string{
								...
								"/usr/bin/insights-client --register", // added when insights is enabled
								"restorecon -R /root/.gnupg",          // added when insights is enabled
								"curl -v --retry --proxy proxy-url https://cert.console.redhat.com/api/patch/v3/templates/template-uuid/subscribed-systems --cert /etc/pki/consumer/cert.pem  --key /etc/pki/consumer/key.pem -X PATCH",
