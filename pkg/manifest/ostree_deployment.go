...
func (p *OSTreeDeployment) serialize() osbuild.Pipeline {
	...
	if p.FIPS {
		p.addFiles(&pipeline, osbuild.GenFIPSFiles(), ref)
	...
	if len(p.Files) > 0 {
		p.addFiles(&pipeline, p.Files, ref)
	}
...
func (p *OSTreeDeployment) addFiles(pipeline *osbuild.Pipeline, files []*fsnode.File, ref string) {
