...
func (p *OS) serializeStart(inputs Inputs) error {
	...
	if p.OSCustomizations.KernelName != "" {
		kernelPkg, err := p.packageSpecs.GetPackage(p.OSCustomizations.KernelName)
		if err != nil {
...
func grubStage(p *OS, pt *disk.PartitionTable, kernelOptions []string) *osbuild.Stage {
	if p.OSCustomizations.NoBLS {
		...
		_, err := p.packageSpecs.GetPackage("dracut-config-rescue")
		hasRescue := err == nil
...
func maybeAddHMACandDirStage(packages rpmmd.PackageList, espMountpoint, kernelVer string) ([]*osbuild.Stage, error) {
	ukiDirect, err := packages.GetPackage("uki-direct")
	if err != nil {
