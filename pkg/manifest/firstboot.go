...
func firstbootFileNodes(fbo *firstboot.FirstbootOptions) ([]string, []*fsnode.File, error) {
	...
	if fbo.Satellite != nil {
		...
		f, err := fsnode.NewFile("/usr/local/sbin/osbuild-first-satellite", common.ToPtr(fs.FileMode(0770)), "root", "root", []byte(fbo.Satellite.Command))
		if err != nil {
	...
	if fbo.AAP != nil {
		...
		f, err := fsnode.NewFile("/usr/local/sbin/osbuild-first-aap", common.ToPtr(fs.FileMode(0770)), "root", "root", []byte(aapContent))
		if err != nil {
			return nil, nil, fmt.Errorf("error creating firstboot file node: %w", err)
		}
