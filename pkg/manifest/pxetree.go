...
func (p *PXETree) serialize() osbuild.Pipeline {
	// TODO
	//  - make directory tree (for now just put things at the /
	//  - Copy the kernel and initrd from the os pipeline tree
	//  - Copy the /EFI files from the os pipeline tree to /EFI/...
	//  - squashfs/erofs the os tree into rootfs.img (exclude /boot content)
