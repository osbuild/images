...
func (m *OTKManifest) Serialize(_ map[string][]rpmmd.PackageSpec, _ map[string][]container.Spec, _ map[string][]ostree.CommitSpec, _ map[string][]rpmmd.RepoConfig) (OSBuildManifest, error) {
	customizations, err := otk.NewCustomizationsFile(m.blueprint, m.entrypoint)
	if err != nil {
		return nil, err
	}
	defer customizations.Cleanup()

	cmd := exec.Command("otk", "compile", "--target=osbuild", customizations.Path())
