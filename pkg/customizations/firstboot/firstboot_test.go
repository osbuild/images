...
func TestFirstbootOptionsFromBP(t *testing.T) {
	tests := []struct {
		...
		{
			name: "unnamed-after-number-based-no-collision",
			json: `{"scripts": [
				{"type":"custom","name":"custom-1","contents":"echo first"},
				{"type":"custom","contents":"echo second"}
			]}`,
			want: firstboot.FirstbootOptions{
				Scripts: []firstboot.Script{
					{
		...
		},
		{
			...
			want: firstboot.FirstbootOptions{
				Scripts: []firstboot.Script{
					{
						Filename: "osbuild-first-first",
						Contents: "echo first",
						After:    []string{"sshd.service"},
						Before:   []string{"postgresql.service"},
					},
					{
						Filename: "osbuild-first-second",
						Contents: "echo second",
					},
				},
			},
