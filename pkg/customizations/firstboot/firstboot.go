...
func FirstbootOptionsFromBP(bpFirstboot blueprint.FirstbootCustomization) (*FirstbootOptions, error) {
	...
	nameFunc := func(inputName, prefix string) string {
		// Use number-based name if name was not provided, is a path, the output
		// name would collide, or matches reserved pattern.
		outputName := ""
		if inputName != "" {
		...
		useNumberBased := inputName == "" || !filepath.IsLocal(inputName) ||
			...
			(outputName != "" && slices.Contains(alreadyUsed, outputName))

		if useNumberBased {
			ci++
			for slices.Contains(alreadyUsed, fmt.Sprintf("osbuild-first-%s-%d", prefix, ci)) {
				ci++
			}
			name := fmt.Sprintf("osbuild-first-%s-%d", prefix, ci)
			alreadyUsed = append(alreadyUsed, name)
			return name
		}

		// keep the naming convention consistent with the existing "osbuild-first-boot"
		alreadyUsed = append(alreadyUsed, outputName)
		return outputName
	}
