...
func (ac Client) RegisterGalleryImage(ctx context.Context, resourceGroup, storageAccount, storageContainer, blobName, name, location string, hyperVGen HyperVGenerationType, architecture arch.Arch) (*GalleryImage, error) {
	...
	// Image is just a container for gallery image versions
	img, err := ac.createGalleryImage(ctx, resourceGroup, location, galleryImage.Gallery, hyperVGen, architecture, fmt.Sprintf("%s-img", name))
	if err != nil {
		return nil, err
	}
	if img.Name == nil {
...
func (ac Client) createGalleryImage(ctx context.Context, resourceGroup, location, gallery string, hyperVGen HyperVGenerationType, architecture arch.Arch, name string) (*armcompute.GalleryImage, error) {
...
func (ac Client) deleteGalleryImageDefinition(ctx context.Context, gi *GalleryImage) error {
	if gi.ImageDef == "" {
		return nil
	}
