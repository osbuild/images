...
func waitForSystem(timeout time.Duration, logger *log.Logger) error {
	if timeout <= 0 {
		return nil
	}
	...
	if err := runningWait(waitCtx, logger); err != nil {
		...
		if errors.Is(err, context.DeadlineExceeded) {
			if activatingUnits := getActivatingUnits(context.Background()); activatingUnits != "" {
...
func runChecks(checks []check.Check, results chan *Result, config *buildconfig.BuildConfig, quiet bool) {
...
func main() {
	...
	if err := waitForSystem(*waitTimeout, logger); err != nil {
		logger.Println("Check completed with return code", 2)
		os.Exit(2)
	}
	...
	runChecks(checks, results, config, *quiet)
	wgCollect.Wait()

	if len(flag.Args()) > 0 {
		fmt.Fprintf(os.Stderr, "Warning: Unexpected argument: %q, use -config instead\n", flag.Arg(0))
	}

	logger.Printf("Check completed with config %q and return code %d", *configFile, returnCode)
	os.Exit(returnCode)
}
