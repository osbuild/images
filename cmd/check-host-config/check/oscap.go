...
func parseOSCAPResults(ctx context.Context, log Logger, filename string) (score string, failedHighSeverityRules []string, err error) {
	data, err := mockos.ReadFileContext(ctx, log, filename)
	if err != nil {
...
func (o OpenSCAPCheck) Run(ctx context.Context, log Logger, config *buildconfig.BuildConfig) error {
	...
	scoreStr, failedRules, err := parseOSCAPResults(ctx, log, "results.xml")
	if err != nil {
