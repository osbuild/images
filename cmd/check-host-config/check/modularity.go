...
func (m ModularityCheck) Run(ctx context.Context, log Logger, config *buildconfig.BuildConfig) error {
	...
	// Get list of enabled modules from dnf
	out, err := cos.ExecContext(ctx, log, "dnf", "module", "list", "--enabled")
	if err != nil {
	...
	if len(lines) < 5 {
		return Fail("unexpected dnf module list output format")
	}
