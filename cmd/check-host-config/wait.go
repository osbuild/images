...
func runningWait(timeout time.Duration, ticks time.Duration) error {
	...
	for {
		...
		if time.Since(startTime) > timeout {
			return fmt.Errorf("timeout before waiting for running state: exceeded %v", timeout)
		}

		stdout, _, err := check.ExecString("systemctl", "is-system-running")
		if err != nil {
		...
		case "initializing", "starting":
			...
			continue
		case "running":
			log.Printf("System is running\n")
			return nil
		case "degraded":
			log.Printf("System is degraded\n")
			return fmt.Errorf("systemctl returned degraded output")
		case "":
			log.Printf("System is in unknown state\n")
...
func listBadUnits() string {
	stdout, _, err := check.Exec("systemctl", "list-units", "--state=activating,failed", "--plain", "--no-legend", "--no-pager")
	if err != nil {
		return ""
	}
	out := stdout
