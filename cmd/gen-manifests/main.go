...
func (bc BuildConfigs) needsSkipping(distro string, cfg *buildconfig.BuildConfig) bool {
		...
		for _, archName := range distroArches {
			...
			for _, imgTypeName := range daImgTypes {
				...
				for _, itConfig := range imgTypeConfigs {
					if configs.needsSkipping(distribution.Name(), itConfig) {
						fmt.Printf("Skipping %s for %s/%s\n", itConfig.Name, imgTypeName, distribution.Name())
						continue
					}
