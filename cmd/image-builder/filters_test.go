...
func TestDistroFilter(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// name: prefix (exact matches only)
		{[]string{"name:bar"}, "test-distro-1", "test_arch3", "qcow2", false},
		{[]string{"name:test-distro-1"}, "test-distro-1", "test_arch3", "qcow2", true},
		{[]string{"name:test-distro"}, "test-distro-1", "test_arch3", "qcow2", false},
		// arch: prefix
		// XXX: should we be exact there? what do users expect?
		{[]string{"arch:amd64"}, "test-distro-1", "test_arch3", "qcow2", false},
		{[]string{"arch:test_arch"}, "test-distro-1", "test_arch3", "qcow2", true},
		...
		{[]string{"name:test-distro-1", "type:ami"}, "test-distro-1", "test_arch3", "qcow2", false},
		{[]string{"name:test-distro-1", "type:qcow2"}, "test-distro-1", "test_arch3", "qcow2", true},
		{[]string{"name:test-distro-1", "arch:amd64", "type:qcow2"}, "test-distro-1", "test_arch3", "qcow2", false},
	} {
		...
		require.NoError(t, err)
		ff, err := main.NewFilters(tc.searchExpr)
		require.NoError(t, err)
