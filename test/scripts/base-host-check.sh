...
check_ca_cert() {
    ...
    cn=$(jq -r '.blueprint.customizations.cacerts.pem_certs[0]' "${config}" | openssl x509 -noout -subject | sed 's/.*CN=//' | sed 's/ /_/g')
    ...
    if ! [ -e "/etc/pki/ca-trust/source/extracted/pem/directory-hash/${cn}.pem" ]; then
        ...
        find /etc/pki/ca-trust/source/extracted/pem/directory-hash
        exit 1
    fi
}
