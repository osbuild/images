...
@pytest.mark.parametrize("distro,arch,img_type,config_name", [tcase.split(",") for tcase in _test_cases()["build_only"]])
def test_build_only(distro,arch,img_type,config_name):
...
@pytest.mark.parametrize("distro,arch,img_type,config_name", [tcase.split(",") for tcase in _test_cases()["build_and_boot"]])
def test_build_and_boot(distro,arch,img_type,config_name):
    subprocess.check_call(
        ["./test/scripts/build-image", distro, img_type, f"test/configs/{config_name}.json"])
    build_dir = os.path.join("build", testlib.gen_build_name(distro, arch, image_type, config_name))
    subprocess.check_call(
